...
exports.handler = async (event, context) => {
    try {
        ...
        if (event.httpMethod === 'POST') {
            ...
            // AQUI ESTÁ A CORREÇÃO: Mapeamento explícito dos campos
            const novaLinha = { 
...
// Funções de e-mail (não precisam de mudança)
async function enviarEmailParaAdmin(dados) {
    ...
    await transporter.sendMail({
        ...
        html: `<p>Uma nova solicitação de agendamento foi recebida.</p><h3>Detalhes:</h3><ul><li><strong>Escola:</strong> ${dados.nomeEscola}</li><li><strong>Data da Visita:</strong> ${new Date(dados.dataVisita + 'T12:00:00').toLocaleDateString('pt-BR')}</li><li><strong>Responsável:</strong> ${dados.nomeResponsavel}</li><li><strong>Contato:</strong> ${dados.emailResponsavel}</li></ul><p>Aguardando aprovação no painel de gestão.</p>`,
    });
}
...
async function enviarEmailParaVisitante(dados) {
    ...
    await transporter.sendMail({
        ...
        html: `<p>Olá, ${dados.nomeResponsavel},</p><p>Recebemos com sucesso sua solicitação de agendamento para o dia <strong>${new Date(dados.dataVisita + 'T12:00:00').toLocaleDateString('pt-BR')}</strong>.</p><p>Sua solicitação está sendo analisada. Em breve, você receberá um novo e-mail com a confirmação.</p><p>Qualquer dúvida, entre em contato conosco por este e-mail ou pelo telefone/WhatsApp <strong>(49) 3341-4819</strong>.</p><p>Atenciosamente,</p><p><strong>Coordenação de Extensão, Ensino, Estágios e Egressos</strong><br>IFC Campus Concórdia</p>`,
    });
}
